...
//	private static Log log = LogFactory.getLog(DA_BYZ.class);
	...
	private int msgsReceivedInCurrentRound = 0;
	...
	public void startAlgo(){
		...
		{
			while (msgsReceivedInCurrentRound < noOfProcesses-1) {
				try {
				...
				//System.out.println(msgsReceivedInCurrentRound);
			}
	...
	public void processMessagesReceivedInCurrentRound() {
		synchronized (messagesReceived) {
			List<Message> messagesReceivedCurrentRound = messagesReceived
					.get(currentRound);
	...
	public synchronized String receive(List<Message> msgs) throws RemoteException {	
		//System.out.println("Process " + this.pId + " received " + msgs);
		if (msgs.size() > 0) {
			int messageRound = msgs.get(0).getPath().size() - 1;

			List<Message> messagesReceivedCurrentRound = messagesReceived.get(messageRound);
			messagesReceivedCurrentRound.addAll(msgs);
			synchronized(messagesReceived) {
				messagesReceived.set(messageRound, messagesReceivedCurrentRound);
			}
		}
		
		msgsReceivedInCurrentRound++;
	 ...
	 */
	public void sendMsg(String lieutenant, List<Message> msgs) {
		try {
	...
	public void broadcast() {
			...
			for (String node : processList) {
				List<Message> messagesToSendForNode = new ArrayList<Message>();
				messagesToSendForNode = generateMessagesToSend(node);
				sendMsg(node, messagesToSendForNode);

				List<Message> msgsToSendInCurrentRound = this.messagesToSend
						.get(currentRound);
				msgsToSendInCurrentRound.addAll(messagesToSendForNode);
				this.messagesToSend.set(currentRound, msgsToSendInCurrentRound);
			}
	...
	public List<Message> generateMessagesToSend(String node) {
		...
		if (currentRound == 0) {
			if (this.pId == 1)
			{
				...
				path.add(pId);
				Message msg = new Message("1", this.currProcess, node, path);
			...
			for (Message message : messagesReceivedPreviousRound) {
				int destProcessNumber = Integer
						.parseInt(node.split("Process")[1]);
				...
				if (!oldPath.contains(destProcessNumber)) {
					...
					Message msgToSend = new Message(message.getVal(),
							this.currProcess, node, newPath);

					if (this.pId > noOfProcesses
							- OutputSettings.noOfFaultyProcesses + 1) {
